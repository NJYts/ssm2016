...
public class ExerciseAll {
    ...
    /**
     * 冒泡排序：通过一个双重循环实现，每次循环下标相邻的两个数进行比较交换，最后会将最大值或者最小值换到结尾，直到有序。
     ...
     * 快速排序 冒泡排序的一种改进。
         它的基本思想是：
         通过递归进行实现。每一轮排序以第一个数为基准值，将要排序的数据分割交换成左右两部分，
         其中一部分的所有数据都比另外一部分的所有数据都要大，再将基准值与两部分相遇位置的值互换，
         这样就确定了基准值的顺序位置。然后再按此方法分别对左右两部分进行排序，
         直到待分割的数据只包含一个元素，就是带分割的左右边界相遇，以此达到整个数据变成有序序列。
         从大到小
     * @param left
     * @param right
     */
     ...
     * 选择出最大或者最小的值，然后将固定元素与最大或者最小值互换位置。继续下次循环，知道外层循环结束。
