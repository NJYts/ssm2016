...
public class ExerciseAll {
    ...
    /**
     * 冒泡排序：通过一个双重循环实现，每次循环下标相邻的两个数进行比较交换，将最大值或者最小值换到结尾，直到有序。
     ...
     * 快速排序 冒泡排序的一种改进。
         ...
         这样就确定了基准值在数列中的顺序位置。然后再按此方法分别对左右两部分进行排序，
         直到待分割的数据只包含一个元素，也就是待分割的数列的左右边界相遇，以此达到整个数据变成有序序列。
         从大到小
     * @param left
     * @param right
     */
     ...
     * 选择出最大或者最小的值，然后将固定位置上的元素与最大或者最小值互换位置。然后继续下次循环，直到外层循环结束。
